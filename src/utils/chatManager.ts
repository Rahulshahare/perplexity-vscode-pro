...
export class ChatManager {
  ...
  public async sendMessage(
    ...
    try {
      ...
      session.messages = session.messages.filter(m => m.id !== session.messages[session.messages.length - 1].id);
  ...
  public async switchToSession(sessionId: string): Promise<void> {
    if (this.sessions.has(sessionId)) {
      ...
      this.eventEmitter.fire(session);
      Logger.debug(`Switched to session: ${sessionId}`);
    } else {
  ...
  public async deleteSession(sessionId: string): Promise<void> {
    if (this.sessions.delete(sessionId)) {
      if (this.currentSessionId === sessionId) {
        ...
        this.currentSessionId = remainingSessions.length > 0 ? remainingSessions[0].id : null;
      }

      await this.saveSessions();
      this.eventEmitter.fire(this.getCurrentSession());
  ...
  private async saveSessions(): Promise<void> {
    try {
      ...
      await this.context.globalState.update('perplexityChat.sessions', sessionsData);
